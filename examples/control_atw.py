...
async def control_atw_device(client: MelCloudHomeClient, device):
    ...
    try:
        ...
        if current_state:
            ...
            print(
                f"Current Zone 1 Temperature: {current_state.get('SetTemperatureZone1', 'Unknown')}"
            )
            print(
                f"Current Tank Temperature: {current_state.get('SetTankWaterTemperature', 'Unknown')}"
            )
        ...
        response = await client.set_device_state(
            device.id, device.device_type, {"power": True, "setTemperatureZone1": 22.0}
        )
        ...
        if device.capabilities.has_hot_water:
            ...
            response = await client.set_device_state(
                device.id, device.device_type, {"setTankWaterTemperature": 55}
            )
            print(f"✓ Response: {response}")
        else:
        ...
        if device.capabilities.has_hot_water:
            ...
            response = await client.set_device_state(
                device.id, device.device_type, {"forcedHotWaterMode": True}
            )
            print(f"✓ Response: {response}")
            ...
            response = await client.set_device_state(
                device.id, device.device_type, {"forcedHotWaterMode": False}
            )
            print(f"✓ Response: {response}")
...
async def main():
    ...
    try:
        async with MelCloudHomeClient() as client:
            ...
            print(
                "Note: Changes may take a few minutes to be reflected in the MelCloud app."
            )
